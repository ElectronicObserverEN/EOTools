...
public partial class ShipClassManagerViewModel : ObservableObject
{
    ...
    public async Task ImportFromAPI()
    {
        ...
        List<ShipModel> ships = db.Ships.Where(s => s.ShipClassId != null).ToList();
        ...
        {
            if (!shipClass.Exists(c => c.ApiId == ship.ShipClassId))
            {
                string nameJapanese = GetShipClassUntranslated((int)ship.ShipClassId!, ship.ApiId);

                ShipClassModel model = new()
                {
                    ApiId = (int)ship.ShipClassId!,
                    NameJapanese = nameJapanese,
                    NameEnglish = translationService.Class(nameJapanese)
                };

                shipClass.Add(model);
                await db.ShipClass.AddAsync(model);
            }
        }
